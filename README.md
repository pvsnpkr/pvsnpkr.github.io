...
## NP-완비
### 3-SAT
NP-완비의 대표주자이고 모든 문제 환원의 기준이다.3-충족 가능성 문제(3-SAT, Boolean satisfiability problem)는 참 또는 거짓의 진리 값을 가질 수 있는 3개의 문자(literals) 또 는 변수(variables) p,q,r 이 논리합(disjunction, OR, ∨)으 로 연결된 절(clause, c) 몇 개가 논리곱(conjunction, AND, ∧)으로 구성된 부울 수식 f가 참이 되도록 문자 의 진리 값을 구할 수 있는가의 문제이다. 즉, 수식 f 가 충족 가능(satisfiable)이란 수식 f가 참이 되기 위해 모든 절이 참이 되도록 문자의 진리 값을 얻어야 하며, 모든 가능한 참인 경우 중 한 가지 이상만 얻으면 된다. 
