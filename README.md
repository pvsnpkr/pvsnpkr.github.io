...
P=NP는 NP문제를 P문제처럼 해결할 수 있는가의 문제이다. 결정론적 문제 P는 답을 찾는데 다항시간이 걸리는 문제이고 비결정론적 NP는 답이 주어지면 다항시간내 확인할 수 있는 문제이다. P가 NP에 속하는 건 자명하다. NP에 속하는 모든 문제는 결정론적 튜링 기계를 사용하여 다항 시간 내에 부울 만족 가능성 문제로 환원 될 수 있다. NP-완전은 NP에 속하고, NP의 모든 문제로부터 다항시간 환원 가능한 문제이다. 따라서 NP-완전 문제중 한가지만 해결해도 모든 NP-완전이 해결되며 P=NP 임이 증명된다.
<br>
<br>
## NP-완전
### 3-SAT
